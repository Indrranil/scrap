...
def test_get_machine_properties():
    ...
    assert "properties" in data  # nosec B101
    assert data["total"] == 1  # nosec B101

    property_data = data["properties"][0]
    assert property_data["property_label"] == "IP Address"  # nosec B101
    assert property_data["property_key"] == "ip_address"  # nosec B101
    assert property_data["property_type"] == "machine"  # nosec B101
...
def test_get_pipeline_input_properties():
    ...
    assert data["total"] == 2  # nosec B101 # Weight property + CLD barcode

    # Check if CLD barcode is included
    cld_barcode_exists = any(
        prop["property_key"] == "cld_barcode" for prop in data["properties"]
    )
    assert cld_barcode_exists  # nosec B101
...
def test_invalid_property_type():
    ...
    assert "Invalid property type" in response.json()["detail"]  # nosec B101
...
def test_property_grouping():
    ...
    weight_props = [
        prop for prop in data["properties"] if prop["property_label"] == "Weight"
    ]
    assert len(weight_props) == 1  # nosec B101
...
def test_inactive_properties_excluded():
    ...
    inactive_exists = any(
        prop["property_label"] == "Inactive Property" for prop in data["properties"]
    )
    assert not inactive_exists  # nosec B101
...
def test_database_error_handling(monkeypatch):
    ...
    assert "Error fetching properties" in response.json()["detail"]  # nosec B101
