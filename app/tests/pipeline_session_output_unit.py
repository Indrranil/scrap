...
def setup_application(db_session):
    ...
    app = (
        ...
        .filter_by(name="Test Application", is_usable=1)
        .first()
    )
...
def setup_pipeline_output(db_session, setup_pipeline_session):
    ...
    # Verify in database
    db_output = db_session.query(PipelineSessionOutput).filter_by(id=output.id).first()
...
def test_create_pipeline_session_output_unit(
    test_db, db_session, setup_pipeline_output, setup_general_property
):
    ...
    assert db_unit is not None  # nosec B101
    assert db_unit.pipeline_session_output_id == setup_pipeline_output.id  # nosec B101
    assert db_unit.property_reference_id == setup_general_property.id  # nosec B101
    assert db_unit.output_key == "test_key"  # nosec B101
    assert db_unit.output_value == "test_value"  # nosec B101
    assert db_unit.status == "idle"  # nosec B101
...
def test_update_pipeline_session_output_unit(
    ...
    # Update the unit
    update_data = {"output_value": "new_value", "status": "success"}

    response = client.patch(
        f"/v1/pipeline-session-output-unit?unit_id={unit.id}", json=update_data
    )
    print(f"\nResponse Status: {response.status_code}")
    print(f"Response Content: {response.text}")
    ...
    db_session.expire_all()
    updated_unit = (
        db_session.query(PipelineSessionOutputUnit).filter_by(id=unit.id).first()
    )
    ...
    assert updated_unit.output_value == "new_value"  # nosec B101
    assert updated_unit.status == "success"  # nosec B101
...
def test_batch_update_pipeline_session_output_units(
    ...
    created_units = (
        db_session.query(PipelineSessionOutputUnit)
        .filter_by(pipeline_session_output_id=output_id)
        .all()
    )
    print("\nInitial Database State:")
    ...
    # Batch update
    update_data = {"status": "ready", "output_value": ""}
    ...
    assert result["updated_count"] == 3  # nosec B101
    ...
    for unit in updated_units:
        print(f"Unit ID: {unit.id}, Value: {unit.output_value}, Status: {unit.status}")
        assert unit.status == "ready"  # nosec B101
        assert unit.output_value == ""  # nosec B101
...
def test_batch_update_with_output_key_filter(
    ...
    params = {"pipeline_session_output_id": output_id, "output_key": "coding"}
    ...
    assert response.status_code == 200  # nosec B101
    result = response.json()
    assert result["updated_count"] == 1  # nosec B101
    ...
    coding_units = (
        db_session.query(PipelineSessionOutputUnit)
        .filter_by(pipeline_session_output_id=output_id, output_key="coding")
        .all()
    )
    other_units = (
        db_session.query(PipelineSessionOutputUnit)
        .filter_by(pipeline_session_output_id=output_id, output_key="perforation")
        .all()
    )
    ...
    assert all(
        unit.status == "ready" and unit.output_value == "" for unit in coding_units
    )  # nosec B101
    assert all(
        unit.status == "idle" and unit.output_value == "value2" for unit in other_units
    )  # nosec B101
...
def test_get_pipeline_session_output_unit(
    ...
    assert fetched_unit["id"] == unit.id  # nosec B101
    assert fetched_unit["output_key"] == "test_key"  # nosec B101
    assert fetched_unit["output_value"] == "test_value"  # nosec B101
...
def test_get_all_pipeline_session_output_units(
    ...
    response = client.get(
        f"/v1/pipeline-session-output-unit?pipeline_session_output_id={setup_pipeline_output.id}"
    )

    assert response.status_code == 200  # nosec B101
    result = response.json()
    assert result["total"] == 3  # nosec B101
    assert len(result["items"]) == 3  # nosec B101
