...
async def get_all_pipelines(db: Session = Depends(get_db)):
    try:
        ...
        for pipeline in pipelines:
            ...
            items.append({**vars(pipeline), "properties": properties})

        return {"total": len(items), "items": items}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching pipelines: {str(e)}"
        )
...
async def get_pipeline(pipeline_id: int, db: Session = Depends(get_db)):
    try:
        ...
        return {**vars(pipeline), "properties": properties}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching the pipeline: {str(e)}"
        )


@router.patch("/{pipeline_id}", response_model=Pipeline)
async def update_pipeline(
    pipeline_id: int, pipeline: PipelineCreate, db: Session = Depends(get_db)
):
