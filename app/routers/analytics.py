...
def get_cumulative_count(start_time: int, end_time: int, output_key: str, db: Session = Depends(get_db)):
    query = SQLFactory.get_query(SQLQuery.CUMULATIVE_COUNT, start_time=start_time, end_time=end_time, output_key=output_key)
...
def get_cumulative_count(start_time: int, end_time: int, pipeline_id: int, db: Session = Depends(get_db)):
