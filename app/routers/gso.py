...
def list_pending(
    ...
    """List transfers awaiting GSO approval."""
...
def list_history(
    ...
    """List past GSO approval decisions."""
    date_to_ts = _parse_date(date_to)
    if date_to_ts:
        date_to_ts += 86399
    total, items = transfer_service.list_transfers(
...
def approve_dispatch(
    ...
    return transfer_service.approve_gso(db, user["gso_id"], body)
...
def reject_dispatch(
    ...
    user = get_current_user(request)
    return transfer_service.reject_gso(db, user["gso_id"], body)
