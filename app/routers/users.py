...
import json
from schemas.user import UserCreate,UsersListResponse,UserResponse,UserUpdate
        ...
        if user_data.roles:
            try:
                ...
                for role_name in current_role_names:
                    if role_name not in user_data.roles and role_name in available_role_dict:
                        roles_to_remove.append(available_role_dict[role_name])
                
                # Remove old roles
                if roles_to_remove:
                    keycloak_admin.delete_realm_roles_of_user(user_id=user_id, roles=roles_to_remove)
                
                # Add new roles
                if roles_to_add:
                    keycloak_admin.assign_realm_roles(user_id=user_id, roles=roles_to_add)

                if invalid_roles:
                    print(f"Warning: Following roles do not exist: {', '.join(invalid_roles)}")
