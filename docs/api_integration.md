...
| **Material sales** | Security records sales via `POST /v1/security/sales` (deducts inventory immediately, `status: pending`) |
| **Vendors** | Extended profile (`vendor_code`, `email`, `contact_person`, `phone`, `address`, `gst_number`); Security can `GET /v1/vendors` |
...
- **Admin** — manages plants, scrapeyard config, reporting, vendors, and legacy sales
